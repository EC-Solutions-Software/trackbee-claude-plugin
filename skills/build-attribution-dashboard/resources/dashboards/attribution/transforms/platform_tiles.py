...
Reads raw MCP outputs (get_dashboard_overview, get_meta_campaign_insights,
...
def _build_other_tile(key: str, overview: dict, provider_string: str) -> dict:
    """Compose a tile for a platform that has no campaign-insights MCP call.
...
def transform(inputs: dict, config: dict) -> dict:
    ...
    Args:
        inputs: ``{"overview": <ov>, "meta": <m>, "google": <g>}``.
                Each value may be the unwrapped MCP payload or missing
