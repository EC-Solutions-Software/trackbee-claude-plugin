...
(function () {
  ...
  var STATUS = (window.TB_DATA && window.TB_DATA.data_status) || {};

  function el(id) { return document.getElementById(id); }
  function setHTML(id, html) { var n = el(id); if (n) n.innerHTML = html; }

  // Render a plain-language "Data unavailable" notice in the host node.
  // Returns true if a notice was rendered (i.e. the section should skip
  // its normal render path); false otherwise.
  function renderUnavailable(hostId, section) {
    var st = STATUS[section];
    if (!st || st.available !== false) return false;
    var host = el(hostId);
    if (!host) return true;
    var reason = (st.reason || '').replace(/</g, '&lt;');
    host.innerHTML =
      '<div class="data-unavailable" role="status">'
      + '<div class="data-unavailable-title">Data unavailable</div>'
      + '<div class="data-unavailable-body">' + reason + '</div>'
      + '</div>';
    return true;
  }
  ...
  function exec(takeaways, label) {
    if (renderUnavailable('execTakeaways', 'exec_takeaways')) { var t = el('execWindowTag'); if (t) t.textContent = label || ''; return; }
  ...
  function blended(b) {
    if (renderUnavailable('kpiBlended', 'blended')) return;
    b = b || {};
    var tiles = [
  ...
  function platforms(plats) {
    if (renderUnavailable('platformTiles', 'platforms')) return;
    plats = plats || {};
  ...
  function attribution(channels) {
    if (renderUnavailable('attrBody', 'channels')) return;
    channels = channels || [];
  ...
  function caveatLine(win) {
    ...
    if (!node) return;
    var ccy = (window.TB_DATA && window.TB_DATA.store && window.TB_DATA.store.currency) || '';
  ...
  function journeys(journeysData) {
    if (renderUnavailable('journeyKpis', 'journeys')) { var fb = el('journeyFallback'); if (fb) fb.innerHTML = ''; var ci = el('cooccurInsights'); if (ci) ci.innerHTML = ''; var ji = el('journeyInsights'); if (ji) ji.innerHTML = ''; return; }
