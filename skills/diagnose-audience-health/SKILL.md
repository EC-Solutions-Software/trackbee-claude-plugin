...
- **Net new reach** (Meta only): The most direct signal. `get_meta_ad_insights` returns `net_new_reach` per ad — users reached who were NOT reached in the prior 90 days. This is different from the standard "reach" in Ads Manager, which only deduplicates within the selected date range. Compare `net_new_reach` to total `reach`: if the ratio is low, the ad is mostly re-showing to existing audience. Also check `cost_per_net_new_reach` — rising cost means each new person reached costs more
...
3. **Custom optimisation goals** — the campaign objective itself can limit audience expansion. Lower priority than creative work; check creatives first.

Also check **attribution windows** for brand awareness impact:

- High `purchases_1d_view` relative to click-based conversions suggests the ad drives brand recall — people see the ad, don't click, but buy later. This indicates upper-funnel reach is still working even when click metrics are low.
- If `purchases_1d_view` is dropping alongside net new reach, the ad is losing reach impact across both new and existing audiences.
...
- **Meta**: `new_customer_purchases` and `returning_customer_purchases` per campaign and ad (from TrackBee Tracking's custom conversion events — null if not configured on the ad account)

Use this to validate whether the audience is truly fresh:

- **Declining new customer ratio**: If new / total drops over time, the campaign is increasingly re-converting existing customers — a sign of audience saturation even if total conversions look stable.
- **New customer CAC rising**: If spend / new_customer_purchases (or conversions) is trending up, acquiring new buyers is getting more expensive — the easy-to-reach audience is tapped out.
- **Cross-reference net new reach with new customer purchases**: For Meta ads specifically, low net new reach + low new_customer_purchases is a double confirmation of audience exhaustion. If net new reach is low but new_customer_purchases stays stable, the ad is still converting the available fresh reach.

### 8. Present the diagnosis

Structure the response as:

1. **Verdict**: Clear statement — "Your Meta campaigns show the saturation pattern" or "No saturation pattern — the CPM increase has a different cause"
...
4. **Recommendations**:
   ...
   - Shift budget to platforms or campaigns with lower frequency and higher CTR
...
- New customer purchases (Meta): orders from first-time buyers, from TrackBee Tracking's custom conversion event. Null when not configured on the ad account.
- Returning customer purchases (Meta): orders from repeat customers, from TrackBee Tracking's custom conversion event. Null when not configured.
