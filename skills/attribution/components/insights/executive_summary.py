...
Inputs:
    ...
    config["store_currency"] — ISO code used to format monetary values.
"""
...
def takeaways(inputs: dict, config: dict | None = None) -> list[str]:
    ...
    out.append(
        f"Generated <strong>{_money(revenue, currency)}</strong> in revenue from "
        f"<strong>{int(orders):,}</strong> orders on "
        f"<strong>{_money(ad_spend, currency)}</strong> of ad spend "
    ...
    if over_reporters:
        ...
        out.append(
            ...
            f"<strong>{_money(er['rev_tb'], currency)}</strong> in assisted revenue with no media spend — "
