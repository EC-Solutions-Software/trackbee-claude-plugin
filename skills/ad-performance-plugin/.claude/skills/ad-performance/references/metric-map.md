...
## Meta (`get_meta_campaign_insights` / `get_meta_ad_insights`)
...
## Google (`get_google_campaign_insights` / `get_google_ad_insights`)
...
| CPM               | `average_cpm`          | |
| CTR               | `ctr` × 100            | Google returns as fraction (0–1), multiply by 100 |
| CPC               | `average_cpc`          | |
| Clicks            | `clicks`               | |
...
## Store-level KPIs (`get_dashboard_overview`)
...
| Frequency ≥ 3.5 AND ROAS still OK       | 🔄 Refresh creative       |
| ROAS < 1.2 AND Spend > £200             | ⏸️ Review / pause        |
| Google ROAS ≥ 4.0 AND Spend > £50      | 📈 Scale aggressively     |
