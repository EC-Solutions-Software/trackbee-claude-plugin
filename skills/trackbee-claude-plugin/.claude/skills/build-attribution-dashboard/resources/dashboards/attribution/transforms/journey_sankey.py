...
def _render_sankey_svg(view: dict) -> str:
    ...
    if not labels or not sources:
        return (
            '<div style="padding:24px;color:#696E7C;background:#FAFAFA;'
    ...
    for i in range(n):
        ...
        plat = parsed[i][0]
        color = _NODE_COLORS.get(plat, "#696E7C")
        h = max(1.0, y1 - y0)
        rect_tip = (
...
def transform(inputs: dict, config: dict) -> dict:
    ...
    return {
        ...
        # browser — they're a documented private side-channel so the
        # journey insights module can read the same paths the SVG was built
        # from. (See CLAUDE.md §4 on the one-key contract exception.)
        "_paths": unioned,
